...
| **P09** | DONE | 2026-08-02: устранён blocker P10 — funnels передаётся как компактная `columns+rows`-проекция с группировкой повторяющихся сегментов, а кандидаты P06 резервируются до связанного контекста без повышения byte-cap; fixture Pognali: пакет <100 КБ, C06/S06 включены, omitted=0. Целевые тесты: **46 passed**. |
| **P10** | BLOCKED | 2026-08-02: `transform -> compute -> build_input_pack` выполнен без LLM; canonical подтверждает open/submit/both = 3002/634/620, CRM = 1357 оплаченных броней и 24 281 956 ₽ выручки, gross-расход = 1 580 161,44 ₽, экономика и 5 рядов сезонности рассчитаны. Приёмка не пройдена: input pack = 175 888 байт (аудит; файл с system prompt = 179 681 байт), все 638 кандидатов исключены, C06/S06 отсутствуют; D11 неожиданно рассчитан как `A+B`/`HIGH` вместо постоянного `permanent_LOW`/confidence cap. Требуется отдельная правка вне `allowed_files` P10 в P09 byte-cap/компактизации и контракте деградации D11. |
