...
| **P03** | DONE | 2026-08-01: `spend_components` собирает canonical и fixed-компоненты в gross-суммах без повторной VAT-нормализации; `cost_summary.json` содержит помесячные строки, итоги компонентов, каналов, месяцев и общий итог, а отсутствие canonical/source_tag явно отражается в limitations. Целевые тесты: **25 passed**. |
...
| **P09** | DONE | 2026-08-01: analyze использует только columnar-кандидаты P06 и компактные funnels/economics/seasonality; mixed unavailable фильтруется построчно, пакет имеет детерминированный byte-cap, coverage и причины исключений; status/confidence/money_category закреплены enum в structured output, сегменты группируются до лимита. Целевые тесты: **45 passed**. |
