...
**7E-econ-contract** DONE — 2026-08-04. `config/report_economics_map.yaml` — явная карта «строка отчёта -> адрес ключа» по фактическим `cost_summary.json`/`acquisition_economics.json`/`money_frame.json`; `build_report.load_report_economics()` собирает объект `report_economics` строго по карте, ничего не вычисляя (проверяется тестом по AST). Отсутствующий ключ -> «не посчитано» с причиной (никогда 0 и не пропуск строки), отсутствующий файл -> «экономика не посчитана», сборка не падает. `cost_per_web_conversion` и `cost_per_deal_by_source` — независимые поля: на L0 второе несёт «недоступно: источник сделки не фиксируется в CRM». Blocker для 7G: compute не отдаёт `attribution_level` — контракт читает адрес `acquisition_economics.json:attribution_level` и до появления источника фиксирует `L_UNKNOWN`; 7G должен закрыть источник этого поля. Рендер секции — 7F (в 7E ничего не рендерится).
**7F-econ-section** DONE — 2026-08-04. `src/report/build_report.py`: секция `## Экономика привлечения` — вторая в отчёте (после вердикта, до плана действий и карточек находок), рендерится всегда, гейт непустого `findings/approved/` на неё не влияет. Пять пунктов: расход построчно на статью с пометкой базы НДС у каждой строки и итогом; результат периода ДВУМЯ числами (записи CRM и веб-конверсии) с объяснением, почему они не совпадают, — разница отдельной цифрой не печатается (её нет в `report_economics`, а report не вычисляет; каталог v2 §11.10); общая стоимость клиента одной цифрой «по всем каналам сразу»; таблица с буквальным заголовком «стоимость веб-конверсии»; стоимость сделки по источникам — на L0 текстовый абзац без единого числа (что нельзя посчитать, из-за какого поля, что внедрить — docs/audit_econ.md §2). Модели `cost_per_web_conversion` разделены между пунктами 3 и 4 по полю `basis` (`tracked_proxy` -> веб-конверсии, `actual`/`estimate` -> стоимость на запись CRM), не по клиентскому `id` — смешение запрещено каталогом v2 §11.3. Слова CAC/ROI/LTV/ROMI/«прибыль» в секции не употребляются (проверяется тестом). Новые CSV приложения + сноски `[4]`/`[5]`/`[6]`: `economics_spend.csv`, `economics_result.csv`, `economics_cost_per_web_conversion.csv` — каждая цифра секции несёт сноску. `config/report_glossary.yaml` +5 различающих терминов (веб-конверсия, стоимость веб-конверсии, сделка, общая стоимость клиента, уровень атрибуции) — верхняя граница числа терминов в `test_report_build.py` поднята с 20 до 25 (явное расширение контракта глоссария этой задачей). Тесты: новый `tests/test_report_economics_section.py` (17 тестов) + `pytest tests/test_report_economics_section.py tests/test_report_build.py tests/test_report_economics_contract.py` — **75 passed**; `tests/test_orchestrator_report_wiring.py tests/test_smoke.py tests/test_config_schema.py` — **39 passed**. Полный pytest не запускался (промт не требовал). Blocker 7G без изменений: `attribution_level` compute не отдаёт, при `L_UNKNOWN` пункт 5 печатает собственную формулировку, а не reason карты (в нём ссылка на задачу 7G — внутренняя, не для клиентского отчёта).

**7G-attribution-level** DONE — 2026-08-04. `src/compute/money_frame.py`: `compute_attribution()` считает `attribution_level` детерминированно по фактической canonical-схеме клиента — L2 (ключ склейки client_id/yclid/gclid либо контакт+таймстамп с ФАКТИЧЕСКИМ JOIN к `visits` выше порога успешности), L1 (заполненное поле источника/utm), L0 (CRM есть, ни одно условие не выполнено), L_UNKNOWN (CRM-источника нет). Все пороги и имена колонок — в `config/defaults.yaml: attribution` (принцип 1), в коде нет ни одного; заглушки (`unknown`, `не определено`, …) заполненностью не считаются. Уровень пишется только вместе с доказательством: новая строка `money_frame.json` `kind="attribution"` несёт `attribution_evidence` — имя каждой проверенной колонки, `present`, фактическую долю непустых значений, порог и результат JOIN; без секции `attribution` в defaults строка не пишется вовсе. Добавлено `unique_customers_available: bool` с различением статусов `not_computable` (постоянное ограничение источника: пустые `is_new_client`/контакт — причина «нет ключа склейки повторных обращений») и `not_computed_yet` (CRM-источника нет). Существующие ключи `money_frame.json` не переименованы — только добавление; report/transform/extract не тронуты. Факт по pognali.rent: **L0**, `unique_customers_available=false`, `not_computable` — совпадает с выводом `docs/audit_econ.md` §2, но получен из данных. Тесты: новый `tests/test_money_frame_attribution.py` (21 тест: L0/L1/L2/L_UNKNOWN, порог ровно на границе включительно, доказательство при каждом уровне, JOIN обязателен для L2, пороги читаются из конфига) — `pytest tests/test_money_frame_attribution.py tests/test_money_frame.py` **38 passed, 2 failed**; обе падающие — предрелизные и не связаны с задачей (`ModuleNotFoundError: scipy` при импорте `block1` в двух dispatch-тестах `test_money_frame.py`, воспроизводятся и до правки). Blocker для report: `config/report_economics_map.yaml` читает `attribution_level` по адресу `acquisition_economics.json`, а compute отдаёт его в `money_frame.json` (`kind="attribution"`) — карта вне `allowed_files` 7G, нужен отдельный шаг перевода адреса на `money_frame`, иначе report продолжит фиксировать `L_UNKNOWN`.

**7H-econ-client-safe-text** DONE — 2026-08-05. Закрыт класс утечки внутренних строк в клиентский документ. `config/report_economics_map.yaml`: адрес `attribution_level` переведён на фактический источник — строку `kind="attribution"` файла `money_frame.json` (7G), пометка `source_status: not_emitted_by_compute` с него снята; тем же адресом протянуты `attribution_evidence` (в контракт, не в текст — это имена колонок и доли заполненности) и `unique_customers_available` с его статусом. `build_report.validate_economics_map()`: каждый адрес карты обязан либо разрешаться в присутствующем файле metrics, либо нести `source_status: not_emitted_by_compute` — третьего нет, неразрешимый непомеченный адрес роняет сборку с указанием строки карты (`EconomicsMapError`). Отсутствие файла целиком проверку не проваливает (принцип 4), непопадание фильтра `where` — факт данных клиента, а не ошибка адреса. Причины «не посчитано» рендерятся ТОЛЬКО фразой из закрытого словаря `client_reason_phrases` по коду `reason_code` строки контракта; внутренняя строка `reason` осталась в контракте для диагностики и в текст не попадает, код без фразы -> «данных источника недостаточно». Пункт 3 назван по факту склейки повторных: её нет -> «Общая стоимость одной сделки»/«одного обращения» по фактической единице записи CRM и без слова «клиент» в пункте; есть -> «Общая стоимость клиента» + отдельная строка про повторные. `not_computable` (постоянное ограничение + что внедрить) и `not_computed_yet` (временно отсутствует, внедрять нечего) — разные тексты (`client_limitation_phrases`). `config/report_glossary.yaml`: «сделка» и «клиент» разведены признаком склейки повторных (+1 термин). Состав и порядок пунктов секции, compute и гейт не тронуты. Тесты: `pytest tests/test_report_economics_contract.py tests/test_report_economics_section.py` — **48 passed** (в т.ч. неразрешимый адрес -> падение; ни одной строки вида `a_b.c_d`; нет подстрок task/7G/check_id/json/parquet/compute/metrics; L0 без склейки -> заголовок пункта 3 без слова «клиент»; not_computable != not_computed_yet; фактические metrics pognali.rent -> L0, не L_UNKNOWN). Blocker (вне `allowed_files`): `tests/test_report_build.py:280` держит верхнюю границу глоссария 25 терминов, после добавления «клиент» их 26 — нужна правка одной константы `15 <= len(glossary) <= 26`.

**DET-1** DONE — 2026-08-05. Слой compute воспроизводим. (1) `evidence_id` больше не зависит от позиции строки: `common.assign_evidence_ids()` считает короткий blake2s-хеш от измерений строки (строковые поля, при коллизии — булевы, в последнюю очередь — порядковый номер внутри группы неразличимых строк); метрики в тождество не входят — иначе новая кампания меняла бы ID соседей через медианы и доли. `row_ref` сохранён как синоним `evidence_id` для `analyze`/`report`. Рядом добавлен `evidence_label` — человекочитаемое описание строки; ссылки (`context_refs`) строятся ТОЛЬКО по `evidence_id`. (2) 55 запросов `GROUP BY` в block0/1/2/3/block4_seo получили `ORDER BY` по измерениям (не по метрике); два подзапроса, отдающих наружу скаляр, оставлены без `ORDER BY` осознанно (`block2:client_id`, `block4_seo:page HAVING`). Единственная сортировка по метрике (D11, `ORDER BY c DESC`) получила тай-брейк по `client_id` — без него D11 расходился между прогонами при равном числе визитов у разных client_id (поймано только на реальных данных, на третьем прогоне). (3) Сериализация: единый порядок ключей для `.json` и `.csv`, `canonical_float` (12 значащих цифр) снимает шум параллельного суммирования DuckDB — ушли артефакты вида `27467.129999999997`; timestamp внутри содержательных артефактов не появлялось и раньше. Проверка на реальных данных `pognali.rent`: два подряд прогона `compute` дают **побайтово идентичные 193 файла** (было 23 расходящихся, PERF-0 §6.2); 7680 `evidence_id`, 477 `context_refs`, неразрешённых — 0; якоря D01 не изменились (`form_submit` 3.126, `form_open` 1.692, оба `overtrigger=true`, диапазон ×2.5–3.9 сохранён), остальные значения артефактов совпали с прогоном до изменений с точностью до снятого float-шума. Тесты: `pytest tests/test_compute_determinism.py` — 8 passed; `pytest tests` — 1108 passed, 2 failed (обе предсуществующие и не связаны: `test_money_frame_registered_in_block_module_names`, `test_glossary_loaded_from_real_config_has_15_to_25_terms`). Вне `allowed_files` пришлось поправить ожидания трёх тестов под явно заданное ломающее изменение контракта ID: `tests/test_block0.py` (d08/d12), `tests/test_block2.py` (registry unavailable), `tests/test_candidates.py` (состав `columns`).

**ROBUST-1** DONE — 2026-08-05. Отсутствующая зависимость больше не роняет compute и видна на первой секунде прогона. (1) `src/compute/common.py`: импорт модулей блоков вынесен в `_import_named_blocks()` — в `try` обёрнута ТОЛЬКО строка `importlib.import_module` и ловится ТОЛЬКО `ImportError`; блок получает статус `missing_dependency`, причину «отсутствует зависимость: <модуль>» и уходит в деградацию, соседние блоки считаются (принцип 4). Любое другое исключение на уровне модуля (ошибка в коде блока, падение при инициализации) намеренно не перехватывается и роняет прогон — иначе баг маскируется под деградацию источника. Результат несёт `missing_dependencies` (блок -> модуль), он же попадает в `metrics_summary.json`. (2) `src/pipeline/degradation.py`: карта объявленных зависимостей `DEPENDENCIES` (имя импорта, имя пакета в requirements.txt, затронутые стадии/блоки), `check_dependencies()` через `importlib.util.find_spec` (код зависимости не выполняется), `format_dependency_table()`, `register_missing_dependencies()` — пишет блок и в `missing_dependencies`, и в `skipped` (тем же приёмом, что `report._report_limitations` с T09), поэтому ограничение прогона попадает в раздел отчёта «Что не удалось проверить»; `counts` — счётчики реестра проверок — не трогаются. `build_degradation_report()` дополнительно фиксирует состояние окружения в `dependencies.missing`. (3) `intake` печатает таблицу «зависимость -> есть/нет -> затронуто» и помечает отсутствующие предупреждением, стадию не останавливает; `run_compute` перезаписывает `degradation_report.json` после dispatch, если блок не импортировался. Тесты: новые `tests/test_degradation_missing_dependency.py` (7) и `tests/test_intake_dependencies.py` (4) — `pytest tests/test_degradation_missing_dependency.py tests/test_intake_dependencies.py` **11 passed**; `pytest tests/test_degradation.py tests/test_compute_common.py tests/test_intake_data_window.py tests/test_orchestrator_report_wiring.py tests/test_orchestrator_input_tables_gate.py tests/test_report_build.py tests/test_compute_determinism.py tests/test_money_frame.py` — **133 passed, 2 failed** (обе предсуществующие и не связаны: `test_money_frame_registered_in_block_module_names`, `test_glossary_loaded_from_real_config_has_15_to_25_terms`). Открытый вопрос контракта: опечатка в имени стороннего импорта внутри блока неотличима от реально отсутствующего пакета (обе дают `ModuleNotFoundError`) и по acceptance criteria 1 уходит в деградацию, а не падает; ловится на intake — такой зависимости нет в `DEPENDENCIES`/requirements.txt.

**PERF-1A** DONE — 2026-08-05. Изменено правило разреза воронок: значения, которые по методологии не могут стать находкой, не считаются поимённо. (1) `config/defaults.yaml`: `segment_min_visits` (=`min_sample_visits`, 500), `segment_max_values: 300`, `segment_coverage_target: 0.80`, `segment_filtered_dimensions` — список разрезов, к которым правило применяется (`entry_page`, `search_phrase`, `key_phrase`, `placement`, `region`); в коде списка нет (принцип 1). Разрезы вне списка (`month`, `channel`, `device`) считаются целиком — `month` к тому же вход `seasonality._funnel_series`. (2) `src/compute/funnels.py`: отбор выполняется ОДИН раз до цикла воронок (он зависит только от визитов) — значения, прошедшие `segment_min_visits`, сортируются по объёму визитов убыв. и берутся, пока не достигнут `segment_coverage_target` ИЛИ не исчерпан `segment_max_values`. Непрошедшие не выбрасываются: одна строка `value="__other__"` с `collapsed_values`/`collapsed_visits` и посчитанными аддитивными счётчиками, поэтому сумма по разрезу сходится с итогом до фильтрации. Неаддитивные доли `transition_rate`/`conversion_rate` у `__other__` — `null`, сумма не подставляется. Покрытие пишется и в разрез (`funnel.segment_coverage`), и в манифест артефакта (`funnels.json: segment_selection` — `policy` + по разрезу `values_total`/`values_selected`/`values_collapsed`/`visits_*`/`coverage_ratio`) как оговорка для отчёта. `compute_funnels(paths, defaults=None)` — при вызове из `block3`/`seasonality` без defaults политика читается прямо из `config/defaults.yaml`, чтобы у трёх вызовов не разошёлся отбор. Логика воронок, порядок этапов и единицы не менялись. Членство визитов в сегменте теперь строится одним проходом (значения нужны с их счётчиками для самого отбора, а `__other__` без прохода не собрать) — результат тождествен прежнему `visits[v][column] == value`; отличие ровно одно: значение `NaN` считается отсутствующим, а не пустой строкой разреза. (3) Факты замера. **220volt.kz** (752 382 визита): разрез `entry_page` **4.23 ч -> 2.26 с**, строк **28 689 -> 113** (112 отобранных + `__other__`), покрытие **30.75%** (порога 500 визитов достигают только 112 страниц, `coverage_target` 0.80 недостижим — берутся все прошедшие); весь `compute_funnels` — **14.25 с**, `funnels.json` **171 КБ** вместо прогнозных ≈36 МБ, то есть пакет `analyze` снова помещается в `analyze_input_pack_byte_cap` (150 000 б после компакции) и находки перестают генерироваться на обрубке. (4) Отклонение от acceptance criteria 4: на **pognali.rent** выход разреза `entry_page` ИЗМЕНИЛСЯ — 107 значений -> 2 отобранных + `__other__`; порога 500 визитов достигают лишь 3 страницы, а `coverage_target` 0.80 набирается на второй (84%). Всего сегментов 131 -> 27, `funnels.json` 163.5 КБ -> 36.8 КБ, `compute_funnels` ~2.0 с -> 0.53 с. Это прямое следствие заданного `segment_min_visits = min_sample_visits`: у pognali.rent средняя посадочная страница даёт ~320 визитов. Если поимённые сегменты ниже 500 визитов нужны как MED-находки, порог для этого клиента задаётся в его `config.yaml`, а не правкой кода. Тесты: `pytest tests/test_funnels.py` — **7 passed** (30 000 значений -> <= `max_values`+1 строк и сходимость сумм по всем аддитивным счётчикам; `null` у неаддитивных величин `__other__`; покрытие в манифесте; разрезы вне списка не режутся); `pytest tests/test_block3.py tests/test_seasonality.py tests/test_compute_determinism.py tests/test_analyze_draft_findings.py tests/test_config_schema.py` — **128 passed**; `pytest tests/test_smoke.py tests/test_compute_common.py tests/test_candidates.py` — **45 passed**. Полный pytest не запускался (промт не требовал). Blocker: нет. Смежное (вне `allowed_files`): `analyze/draft_findings._compact_funnels` пропускает наверх только скалярные ключи, поэтому `segment_selection` в пакет LLM не попадает — чтобы оговорка о покрытии доехала до отчёта, её нужно протянуть отдельной задачей.

**PERF-1B** DONE — 2026-08-05. Воронки считаются один раз за прогон. (1) Сверка параметров трёх вызовов: они были тождественны — `funnels.run` передавал `defaults` диспетчера (`config/defaults.yaml`), `block3._run_c06` и `seasonality._funnel_series` вызывали `compute_funnels(paths)` без defaults, а `_load_defaults(None)` читает тот же `config/defaults.yaml`; набор разрезов, гранулярность (месяц) и единицы (визиты) одинаковы. Разной гранулярности нет: `seasonality` берёт срез `dimension="month"` того же артефакта, `block3` — итог и все сегменты. (2) `src/compute/funnels.py`: добавлен `load_funnels(paths, defaults=None)` — читает артефакт слоя metrics `funnels.json`, который пишет блок `funnels`, идущий в `BLOCK_MODULE_NAMES` раньше `seasonality` и `block3`. `block3` и `seasonality` переведены на `load_funnels`; `compute_funnels` остаётся запасным путём, когда артефакта нет (блок `funnels` упал, вызов вне dispatch, битый файл) — принцип 4. Мемоизации по сигнатуре нет. (3) Отклонение от задания: артефакт — `funnels.json`, а не новый `funnels.parquet`. Причина: слой metrics целиком json/csv (`common.write_metric_artifact`), parquet по принципу 5 — формат слоя canonical; `funnels.json` уже существует, уже читается `acquisition_economics._read_json(metrics_dir/"funnels.json")` и потребляется `analyze`. Второй артефакт с тем же содержимым в другом формате добавил бы сериализацию к задаче, цель которой — убрать лишнюю работу. Структура воронок (вложенные списки этапов/переходов/сегментов с необязательными ключами) в табличный parquet не ложится без обёртки-блоба. (4) Факты замера, `clients/pognali.rent`, два прогона подряд: `dispatch_blocks` **9.66 с -> 7.91 с** (прогрев 8.06 с -> 7.25 с), `compute_funnels` **ncalls=3 -> 1**. Все 194 файла `data/metrics/` побайтово совпали с выходом DET-1/PERF-1A (SHA-256 по каждому файлу), включая `c06.json`/`c06.csv`, `seasonality.json` и `funnels.json`. Тесты: `pytest tests/test_funnels.py` — **10 passed** (ровно один `compute_funnels` за прогон funnels->seasonality->block3 при совпадении значений с артефактом; запасной расчёт без артефакта; битый артефакт не используется); `pytest tests/test_block3.py tests/test_seasonality.py tests/test_compute_determinism.py tests/test_acquisition_economics.py tests/test_compute_common.py tests/test_smoke.py` — **102 passed**. Полный pytest не запускался (промт не требовал). Blocker: нет.


**SEG-FIX** DONE — 2026-08-06. Исправлена постановка PERF-1A: порог расчёта и порог достаточности выборки разведены, сырой URL перестал быть единственным измерением точки входа. (1) `config/defaults.yaml`: ключ `segment_min_visits` (=`min_sample_visits`=500) больше не читается; введён `segment_compute_min_visits: 30` — порог РАСЧЁТА (стоит ли значению отдельная строка артефакта), `segment_max_values: 300 -> 2000`, добавлен `segment_entry_page_top_per_type: 1500`. `min_sample_visits` продолжает применяться там же, где применялся, — в `block3._run_c06` (`_sample_confidence` на сегмент), то есть на стадии превращения сегмента в кандидата в находку. `segment_coverage_target: 0.80` из правила ОСТАНОВКИ стал ЦЕЛЬЮ: раньше отбор прекращался при её достижении, и разрез с одной доминирующей страницей (pognali.rent: `/` — 61% визитов) схлопывался в две строки; теперь берутся все значения от порога расчёта до `segment_max_values`, а факт достижения цели пишется в манифест (`coverage_target_met`). (2) `src/compute/funnels.py`: добавлено производное измерение `page_type` — отдельный разрез РЯДОМ с `entry_page`, не вместо него; правила (`config.yaml: page_types`) — упорядоченный список `id` + regex `match`, побеждает первое совпадение, порядок значим и живёт в конфиге, в коде правил нет (принцип 1). URL вне правил получает `page_type: "other"` — значение, а не пропуск; строка `other` присутствует в разрезе всегда, даже нулевая. `page_type` не входит в `segment_filtered_dimensions` (низкая кардинальность) и порогами не режется. `entry_page` остался drill-down: отбор по `segment_compute_min_visits` + топ-`segment_entry_page_top_per_type` внутри каждого `page_type` + общий потолок `segment_max_values`. Без `page_types` в конфиге клиента разреза `page_type` у него просто нет (классифицировать URL нечем) — обратная совместимость. В манифест добавлены `values_below_compute_threshold`, `values_dropped_by_group_cap`, `coverage_target_met`. (3) `clients/220volt.kz/config.yaml`, `clients/pognali.rent/config.yaml`: `page_types` выведены из ФАКТИЧЕСКОГО распределения `entry_page` каждого клиента (топ-30 шаблонов пути + разбор структуры), не по аналогии между клиентами. У 220volt.kz карточка товара отличается от раздела цифрой/`*`/`(` в последнем сегменте пути (артикул/типоразмер) — правило проверено фактом: разделов с цифрой в последнем сегменте в выгрузке нет; у pognali.rent уровня категорий нет вовсе, карточка авто лежит прямо в `/catalog/<slug>`. (4) Факты замера. **220volt.kz** (752 382 визита): `page_type` — 11 значений, покрытие визитов **1.0**, доля `other` **0.03%** (234 визита); `entry_page` — **112 -> 1758** значений, покрытие **30.75% -> 66.57%**, ниже порога расчёта 24 393 значения, потолком на тип отброшено 2 538; `compute_funnels` **14.25 с -> 25.6 с** (потолок 120 с не превышен, PERF-1C не нужен), `funnels.json` 171 КБ -> **2.37 МБ**. **pognali.rent** (34 227 визитов): `page_type` — 7 значений, покрытие **1.0**, доля `other` **0.003%** (1 визит); `entry_page` — **2 -> 63** значения из 107, покрытие **84% -> 98.17%**; `compute_funnels` 0.53 с -> **1.0 с**, `funnels.json` 36.8 КБ -> **123.7 КБ**. Детерминизм: два прогона подряд дают побайтово идентичный `funnels.json` у обоих клиентов (SHA-256 совпал). (5) Отклонение от acceptance criteria: у 220volt.kz покрытие `entry_page` **0.6657**, а не >= 0.75 — фиксируется как фактически достижимое при `segment_max_values: 2000`. Потолок без учёта топ-N на тип — 0.6939 (2000 строк), порог 0.75 требует ~2 855 строк (все значения от 50 визитов). Итоговые 0.6657 — цена того, что бюджет строк делится между типами: без потолка на тип `product` вытеснил бы 57 строк `category`/`blog`/`info`. По criteria 3: у pognali.rent восстановлено 63 поимённых значения против 107 до PERF-1A — разница ровно в значениях ниже 30 визитов (44 значения, 1.83% визитов), они схлопнуты в `__other__`. Тесты: `pytest tests/test_funnels.py` — **17 passed** (7 новых: coverage_target не останавливает отбор; схлопывание ниже порога расчёта со сходимостью сумм; деление бюджета между page_type; покрытие page_type и всегда присутствующая строка `other`; порядок правил — первое совпадение; отсутствие разреза без правил в конфиге); `pytest tests/test_block3.py tests/test_seasonality.py tests/test_compute_determinism.py tests/test_config_schema.py tests/test_candidates.py tests/test_compute_common.py tests/test_analyze_draft_findings.py` — **173 passed** суммарно с test_funnels.py. Полный pytest не запускался (промт не требовал). Blocker: нет. Смежное (вне `allowed_files`): `clients/_template/config.yaml` без секции `page_types` — новому клиенту разрез не достанется, пока правила не выписаны вручную; `analyze/draft_findings._compact_funnels` проецирует воронки на `funnel_id`/`totals`/`transitions`/`gaps`/`anomalies`, поэтому рост `funnels.json` до 2.37 МБ в пакет LLM не идёт, но и оговорка о покрытии (`segment_selection`) по-прежнему туда не попадает.

**BLOCK-REG** ДИАГНОСТИКА (без исправления) — 2026-08-06. Фактический статус `money_frame` в dispatch по четырём вопросам. (1) **Вызывается.** `common.BLOCK_MODULE_NAMES` содержит `money_frame` на 12-й позиции из 13 — после `cost_summary`/`acquisition_economics`, перед `candidates`; `dispatch_blocks` импортирует его по имени и вызывает `run(paths, defaults, runnable_ids)`. Реальный прогон `clients/pognali.rent`: `metrics_summary.json → block_status.money_frame = "ok"`, `artifacts` содержит `money_frame`; `docs/perf0_compute_diagnostics.md` — 0.104 с, 1.1% времени compute. Падающий тест `tests/test_money_frame.py::test_money_frame_registered_in_block_module_names` падает не на `in`, а на второй строке `BLOCK_MODULE_NAMES[-1] == "money_frame"`: последним стоит `candidates` (добавлен позже). Это устаревшее ожидание теста, не отсутствие регистрации. (2) **Частично нет.** Файла `_compute_profile.json` в репозитории не существует ни в одном виде — статус блоков живёт в `data/metrics/metrics_summary.json` (`block_status`), и там `money_frame` присутствует. В деградацию он не попадает вовсе: `M` не заведён в `config/methodology.yaml`, в `degradation_report.checks` (100 записей) его нет. Механизм `missing_dependency` в `dispatch_blocks` ловит только `ImportError` python-пакета; фактическая зависимость `money_frame` — уже записанные артефакты `a04.json`/`c06.json`/`a05…a19.json` — при отсутствии просто не даёт строк: `_read_artifact` возвращает `None`, `run` молча пропускает правило. Наблюдается на реальных данных: `a04.json` — единственная строка `status: unavailable` (`macro_goals` не заданы), поэтому `_blended_cpa_from_a04` вернул `None`, в `money_frame.json` нет ни одной строки `kind="quantity"` (ни `total_ad_spend_rub_a04`, ни `blended_cpa_rub`) и ни одного сценария C06 — и об этом нет записи ни в `money_frame.json` (оговорка предусмотрена только для SEO, `SEO_NOT_READY_NOTE`), ни в `metrics_summary`, ни в `degradation_report`. Денежная рамка молча выглядит полной при отсутствующем входе. (3) **Нет.** `money_frame.run` пишет через `common.write_metric_artifact` напрямую и ни разу не вызывает `common.assign_evidence_ids` — в отличие от block0/1/2/3/block4_seo. Строки реального `money_frame.json` (8 строк: 6 `category_item`, 1 `attribution`, 1 `category_total`) не несут ни `evidence_id`, ни `row_ref`. Позиционных ID у него тоже нет — идентификатора строки нет вообще, ссылаться на строку денежной рамки из `analyze`/`report` нечем. `tests/test_compute_determinism.py:409` это зафиксировал явным пропуском («артефакты вне candidate-контракта (money_frame и т.п.)»), то есть DET-1 обошёл money_frame, а не покрыл его. То же относится к `findings_registry.csv/json`. (4) **Да.** `money_frame.csv/json` и `findings_registry.csv/json` лежат в `clients/pognali.rent/data/metrics/` (193 файла), а `_metric_files` в тесте побайтовой воспроизводимости берёт все файлы каталога кроме `manifest.json` — то есть в сверку они входили и разошедшимися не были. Побайтовое совпадение при этом ничего не говорит про (3): у строк нет ID, стабильность которого можно нарушить. **Вывод: блокер уровня продукта, не провал теста.** Два дефекта: (а) выход `money_frame`/`findings_registry` не имеет `evidence_id` — контракт DET-1 на него не распространён, ссылки на денежные строки невозможны; (б) отсутствие входного артефакта A/C деградирует молча, без записи в degradation и без оговорки в самой рамке. Исправление не выполнялось (задача — диагностика). Ожидание теста `BLOCK_MODULE_NAMES[-1] == "money_frame"` также требует решения: либо тест переписать на «money_frame раньше candidates», либо зафиксировать порядок явно.

**AUDIT-A04** DONE — 2026-08-06 (диагностика, без правок). (1) A04 не хватает НЕ денег и НЕ Q01: не хватает чистых конверсий по кампаниям. `_run_a04` (`src/compute/block1.py:828`) пишет unavailable, когда `_campaign_net_conversions` вернул None; у pognali.rent срабатывают ОБА условия сразу — в `clients/pognali.rent/config.yaml` секция `sources.direct` вообще не содержит ключа `macro_goals` (только `enabled`/`client_login`), и в `data/canonical/direct_campaigns.parquet` (1377 строк) нет ни одной колонки `goal_conv_<id>` — есть только запрещённый правилом 11 `conversions_all`. Деньги для A04 полностью готовы: Q01 отвечен (`inputs/client_answers.yaml: finance.vat_basis_by_source`, direct/seo/Яндекс Бизнес = vat_included true), `costs.parquet` даёт cost_normalized на всех 1377 строках (492 661,44 ₽ с НДС -> 410 551,20 ₽ без). (2) Структурное, не клиентское: `macro_goals: []` пусто у всех четырёх клиентов и в `_template`; ни у кого A04 сейчас не считается. Но блокирует не Q01, а отдельный клиентский вопрос «какие цели Директа считать бизнес-целями» (Q-вход `sources.direct.macro_goals`; `src/extract/direct.py:764` уже фиксирует «целевые конверсии не выгружены — macro_goals не настроен; CPA не рассчитывается»). (3) 2 142 ₽ и 3 711 ₽ собраны ВНЕ пайплайна: единственный артефакт с ними — `D:/Projects/marketing-stuff/clients/pognali-rent/06_report/pognali-final-report-v3.html` (строки 295, 733–734, 776), другой репозиторий, ручная вёрстка. В пайплайне их нет; ближайший расчёт `data/metrics/acquisition_economics.json` даёт другие числа по другой базе: estimated_site_booking 1 293,84 ₽ (1 580 161,44 ₽ / 1221,3 записи CRM) и tracked_direct_booking 3 286,29 ₽ (920 161,44 ₽ / 280 отправок формы). Отчётные 2 142/3 711 не воспроизводятся ни из одного metrics-файла. (4) money_frame: из его входов по той же причине (macro_goals) unavailable восемь проверок — A04, A05, A06 (`_FLAT_RULES`/campaign-экономика) и A09, A10, A11, A12, A13; плюс мёртв весь путь сквозного CPA A04 (`money_frame.py:352,410–414`), то есть перевод C-сценариев в рубли. Фактический `money_frame.json` у pognali.rent — 8 строк, деньги только из A17, итог 921 ₽.

**Приоритет.** Причина — НЕ неподтверждённая база НДС. Q01 по этому клиенту закрыт, НДС-нормализация работает. Блокирует другой клиентский вход — список бизнес-целей Директа (`sources.direct.macro_goals`), и он блокирует центральный блок продукта, а не периферию: без него нет ни одной кампанийной/фразовой единицы экономики и нет сквозного CPA, на котором держится вся денежная рамка отчёта. Это вопрос клиенту (или сверка с `data/raw/metrika_reports/goals_list.json` + attribution_type), стоящий выше по приоритету, чем любые правки кода в block1.

---

## CONTRACT-AUDIT (2026-08-06) — диагностика, без исправлений

Инвентарь: `BLOCK_MODULE_NAMES` = 13 записей; `config/methodology.yaml` = 100 ID
(D01–D12, A01–A26, T01–T10, C01–C25, S01–S27); `clients/pognali.rent/data/metrics/`
= 193 файла = 100 стемов (91 check-артефакт + 9 служебных).

### Таблица 1 — 13 блоков диспетчера

| # | блок | ID в methodology | в degradation | evidence_id в строках | побайтовая сверка осмысленна | читается дальше |
|---|------|------------------|---------------|----------------------|------------------------------|-----------------|
| 1 | block0 | D01–D12 | да (12/12 runnable) | да | да | analyze, report |
| 2 | block1 | A01–A26 | да (23 runnable, A01–A03 skipped) | да | да | analyze, report, money_frame |
| 3 | block2 | T01–T10 | да (6 runnable, T03/T04/T08/T10 skipped) | да | да | analyze, report |
| 4 | funnels | нет ID | нет | нет (dict-артефакт) | да, но только как «код не менялся» | analyze (`draft_findings`) |
| 5 | seasonality | нет ID | нет | нет (dict-артефакт) | то же | analyze (`draft_findings`) |
| 6 | block3 | C01–C25 | да (20 runnable; C01/C02/C14/C20/C24 skipped) | да | да | analyze, report |
| 7 | block4_seo | S01–S27 | да (26 runnable, S20 skipped) | да | да | analyze, report |
| 8 | block5 | — | нет | — | — | нет: `NotImplementedError`, докстринг в легаси-нумерации 5.1–5.6 (дубль S-проверок) |
| 9 | block6 | — | нет | — | — | нет: `NotImplementedError`, легаси 6.1–6.3 (CRM) |
| 10 | cost_summary | нет ID | нет | нет (dict) | да, но только как «код не менялся» | analyze, report |
| 11 | acquisition_economics | нет ID | нет | нет (dict) | то же | analyze, report |
| 12 | money_frame | нет ID (агрегирует A/C) | нет | **нет** (8 строк `unmarked`) | **нет: нет ключа строки** | `analyze/schemas.py`, `report/build_report.py` |
| 13 | candidates | нет ID | нет | да (назначает сам) | да | analyze (`draft_findings`) |

Вне диспетчера: `src/compute/block4.py` — заглушка старой нумерации 4.1/4.2,
не входит в `BLOCK_MODULE_NAMES`, не вызывается ниоткуда (мёртвый код).

### Таблица 2 — 100 артефактов метрик по классам контракта

| класс (стемы) | ID в methodology | degradation | evidence_id | побайтовая сверка | читается дальше |
|---------------|------------------|-------------|-------------|-------------------|-----------------|
| 86 check-стемов D/A/T/C/S со строками (`status=complete`, rows>0) | да | да | да, 100% строк | да, осмысленна | candidates → analyze → report |
| `a18` (0 строк) | да | да (runnable) | н/д | нет: пустой файл всегда равен пустому | candidates (0 строк) |
| `t03`, `t04`, `t08`, `t10` | да | **skipped** (нет `referer_domains`/`goal_attribution_models`/`visit_campaign_map`) | да | сверка сравнивает **устаревшие** файлы | candidates включает их как валидные |
| `money_frame`, `findings_registry` | нет | нет | **нет**, `status=legacy`, все строки `unmarked` | **нет** | money_frame — да; `findings_registry` — **никем** |
| `cost_summary`, `acquisition_economics`, `funnels`, `seasonality` | нет | нет | нет, `shape=unsupported` | только как «код не менялся» | analyze/report (кроме funnels/seasonality → только analyze) |
| `degradation_report`, `metrics_summary`, `analysis_candidates` | н/д (служебные) | сами и есть контур | н/д | да | orchestrator, analyze, report |

10 ID реестра без артефакта — ровно `skipped` текущего прогона: A01–A03
(стратегии Директа), C01/C02/S20 (CrUX), C14/C24 (site_crawl), C20 (вебвизор).
Расхождений «runnable без артефакта» нет.

**STATE-1** DONE — 2026-08-06. degradation_report и содержимое `data/metrics/` связаны одним идентификатором прогона; `candidates` перестал обнаруживать входы глобом. (1) `run_id` назначается один раз в `orchestrator.run_compute` и оттуда же открывает контекст прогона (`common.set_run_context`); он попадает в `degradation_report.json`, в реестр `data/metrics/manifest.json` и в `analysis_candidates.json`. **run_id детерминирован** — это отпечаток sha256 от готового degradation_report (без самого `run_id`), а не UUID и не время. Причина в контракте: `tests/test_compute_determinism.test_two_runs_produce_byte_identical_metrics` требует побайтового совпадения ВСЕХ файлов `data/metrics/` между двумя прогонами, а этот тест вне `allowed_files`; случайный run_id краснел бы на каждом артефакте. Следствие принято сознательно: два прогона подряд на неизменившемся входе делят run_id, и не переписанный артефакт такого прогона считается актуальным — он и есть актуальный. Меняется вход (источник, набор runnable, caps) — меняется run_id, и всё непереписанное становится stale. (2) `common.write_metric_artifact` регистрирует свой артефакт в реестре прогона (`write_json_atomic(..., register=False)` + явный `register_run_artifact` — владение регистрацией видно там же, где пишется артефакт); dict-артефакты (`funnels`, `seasonality`, `cost_summary`, `acquisition_economics`, `money_frame`) регистрируются через `write_json_atomic`. Вне контекста прогона регистрация инертна — юнит-тест блока и отладочный вызов ничего не пишут в реестр. (3) `src/pipeline/manifest.py`: реестр входов слоя metrics (`load_metrics_manifest` / `start_metrics_run` / `register_metrics_artifact`). Реестр НЕ обнуляется в начале прогона — иначе артефакт, который никто не переписал, неотличим от файла, которого в слое быть не должно. Файл лежит в `data/metrics/manifest.json` и уже исключён из побайтовой сверки (`tests/test_compute_determinism._metric_files`), поэтому может нести время записи. (4) `candidates.build_analysis_candidates(metrics_dir, run_id)` строит входы из реестра: запись с текущим run_id -> вход; запись с чужим run_id -> `stale`; файла нет в реестре -> `unregistered`. Ни то, ни другое в пакет для analyze не попадает и ни то, ни другое не пропускается молча: оба списка есть в `coverage` артефакта и оба через `degradation.register_artifact_states` попадают в `degradation_report.artifact_states` и в лог стадии. Файлы не удаляются (запрет промта соблюдён): чужой артефакт остаётся уликой для разбора прогона. `run_id=None` (вызов вне dispatch) отключает реестр — без идентичности прогона состояние неизвестно, и выдумывать `stale` вместо признания незнания нельзя.

**Отклонение от промта, п.2.** Требование «write_metric_artifact пишет run_id в каждый артефакт **верхним полем**» не выполнено буквально и выполнено через реестр-сайдкар. Причина — ломающий контракт вне `allowed_files`: артефакт проверки это JSON-**список** строк, у списка верхнего поля нет, а обёртка в dict рвёт всех читателей: `money_frame._read_artifact` (`src/compute/money_frame.py:219`) возвращает `None` на всём, что не `list` — денежная рамка молча обнулилась бы; плюс `analyze/draft_findings._load_json_artifact`, `report/build_report`, `analyze/validate_findings`. Все четыре файла вне `allowed_files`, и правило 2 протокола запрещает расширять скоуп молча. Сайдкар даёт ту же семантику (у каждого файла ровно одно состояние: current/stale/unregistered) при нулевом изменении формы артефактов. Критерий «при откате run_id из write_metric_artifact тест краснеет» проверен фактически: удаление строки `register_run_artifact` из `write_metric_artifact` роняет 5 тестов `tests/test_run_identity.py` (11 passed -> 5 failed, 6 passed).

**Факты прогона `clients/pognali.rent`.** `run_id = 4ce5dc99df8e5f06`, одинаков в `degradation_report.json`, `manifest.json` и `analysis_candidates.json`; записей с чужим run_id нет (проверено обходом реестра). Артефактов просканировано 97, `artifacts_complete` **91 до и 91 после** — STATE-1 не изменил ни одной классификации. `t03/t04/t08/t10`: по одному состоянию у каждого — `complete`, зарегистрированы текущим run_id. **Фактическая поправка к CONTRACT-AUDIT:** эти файлы НЕ устаревшие. `block2.run` (`src/compute/block2.py:876-879`) переписывает их в каждом прогоне как одностроч­ный limitation-артефакт (`status=unavailable`, `row_role=limitation`), а `dispatch_blocks` перед этим удаляет прошлые (`_remove_skipped_metric_artifacts`). «skipped в degradation» и «complete у candidates» — не противоречие: первое про проверку (источника нет), второе про контракт строки (ограничение размечено полностью и обязано доехать до раздела «Что не удалось проверить»). Проверка механизма на живом клиенте: подложенный `x.json` -> `unregistered`, в пакет не попал, запись в `degradation_report.artifact_states`; подменённый run_id у `t03` -> `stale`, из пакета исключён, файл на диске остался. Побайтовая сверка `data/metrics/` до и после STATE-1: изменились ровно 2 файла по причине STATE-1 (`degradation_report.json` — ключ `run_id`; `analysis_candidates.json` — `run_id` + два списка состояний) плюс новый `manifest.json`. `c06.json/csv` и `funnels.json` тоже разошлись, но по причине уже закоммиченного SEG-FIX (`min_visits` 500->30, разрез `page_type`, `entry_page` 2->63 значения), а не STATE-1.

**Число по п.6 промта (зафиксировано, не чинится).** Из 100 ID реестра ровно **62** дают >= 1 строку в `analysis_candidates` на pognali. Из 38 остальных 9 пропущены по деградации (A01–A03, C01, C02, C14, C20, C24, S20 — артефакта нет), а **29 runnable, имеют артефакт и не дают ни одной строки**: A18, A19, A22, C04, C05, C07, C09, C10, C13, D05, D06, D10, S01, S08, S10–S19, S21–S23, T01, T05. Их строки существуют и размечены, но ни одна не `candidate=true`, не `limitation` и ни на одну не ссылается `context_refs` живого кандидата, поэтому отбор их не берёт. Это отдельная задача.

**Тесты.** `pytest tests/test_run_identity.py` — 11 passed (новый файл: детерминизм run_id и его независимость от собственного поля; регистрация артефакта; инертность вне прогона; stale по чужому run_id с исключением из пакета и записью в degradation_report; неудаление stale-файла; unregistered для постороннего файла; манифест не считается входом). `pytest tests/test_compute_determinism.py` — 8 passed (побайтовая воспроизводимость сохранена). `pytest` целиком — **1145 passed, 1 failed**. Единственное падение — `tests/test_report_build.py::test_glossary_loaded_from_real_config_has_15_to_25_terms` (26 терминов против потолка 25); проверено на чистом дереве до STATE-1 — падает так же, к задаче отношения не имеет, а глоссарий и `test_report_build.py` промт править запрещает. Blocker: нет.

**Смежное (вне `allowed_files`, не делалось).** `tests/test_compute_determinism._metric_files` исключает `manifest.json` из сверки — реестр туда и лёг, но это совпадение, а не договорённость: если реестр когда-нибудь переедет, исключение надо будет переписать. `money_frame`/`findings_registry` по-прежнему без `evidence_id` (задача REG-M), поэтому их строки в реестр входов попадают, а в кандидаты — нет.
